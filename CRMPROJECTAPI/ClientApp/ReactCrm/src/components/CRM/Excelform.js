...
export default function Excelform() {
  ...
  const tableScrollBottomRef = useRef(null);
  ...
  useEffect(() => {
    ...
    if (tableScrollTopRef.current && tableScrollBottomRef.current) {
      tableScrollTopRef.current.addEventListener("scroll", syncScroll);
      tableScrollBottomRef.current.addEventListener("scroll", syncScroll);
    }

    return () => {
      if (tableScrollTopRef.current && tableScrollBottomRef.current) {
        tableScrollTopRef.current.removeEventListener("scroll", syncScroll);
        tableScrollBottomRef.current.removeEventListener("scroll", syncScroll);
      }
    };
  }, []);
  const syncScroll = (e) => {
    if (e.target === tableScrollTopRef.current) {
      tableScrollBottomRef.current.scrollLeft = e.target.scrollLeft;
    } else {
      tableScrollTopRef.current.scrollLeft = e.target.scrollLeft;
    }
  };
