...
const AddUsers = () => {
  ...
  const handleRoleFilterChange = (e) => {
    ...
    if (selectedRoleId) {
      const filtered = users.filter((user) => user.roleId.toString() === selectedRoleId)
  ...
  const handleInputChange = (event) => {
    ...
    if (name === "roleId") {
      ...
      const roleWithOnlyBranches = ["da9f9bc0-e58a-4a9f-8d59-183099ce2ba9", "a8c8ea20-7154-4d78-97ea-a4d5cf217a27"]
    ...
    if (validateForm()) {
      if (isEditMode) {
        ...
        putRequest(`/api/users/${currentUserId}`, updateUser)
          .then((res) => {
            const updatedUsers = users.map((user) => (user.userId === currentUserId ? res.data : user))
            setUsers(updatedUsers)
            setFilteredUsers(updatedUsers)
            setShowModal(false)
            resetNewUser()
                  ...
                  {showBranchesAndCategories && (
                    <div className="mb-3">
                      ...
                      <select
                        id="categoriesId"
                        name="categoriesId"
                        className={`form-control ${errors.categoriesId ? "is-invalid" : ""}`}
                      ...
                      {errors.categoriesId && <div className="invalid-feedback">{errors.categoriesId}</div>}
                    </div>
                  )}
                </div>
